                              ...
                              float zMin, float zMax)
{
    ...
    // Populate caches
    for (int i(0); i < cacheCount; i++) {//рисует на глубину 24
        QSurfaceDataArray &cache = m_data[i]; //получаем поверхность

        float stepX = xRange / (float)(columnCount - 1);
        float stepZ = zRange / (float)(rowCount - 1);
        float stepY = yRange / (float)(cacheCount - 1);
        ...
        { //row - строка(1000)
            ...
            {//столбики(800)
                row[k] = QVector3D(float(k)*stepX,(float)i*stepY,(float)(j)*stepZ);
            }
        }
